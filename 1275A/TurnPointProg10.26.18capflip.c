...
task autoFlip ()
{
	...
	{
		while(!flipperEnable && SensorValue[flip] < 1120 && flipperUpOk)
			motor[flipper] = 127;
	}
	flipperEnable = 1;
}
...
task autonomous()
{
	...
	if(SensorValue[jump])
	{
		...
		motor[leftDrive] = -leftSpeed;
		motor[rightDrive] = -rightSpeed;
		waitUntil(getMotorEncoder(leftDrive) < -158);
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
		...
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
		motor[leftDrive] = -leftSpeed * 0.5;
		motor[rightDrive] = -rightSpeed * 0.69;
		while(SensorValue[flip] > 3)
			motor[flipper] = -127;
		motor[flipper] = 0;
		wait1Msec(400);
		...
		motor[leftDrive] = leftSpeed * 0.5;
		motor[rightDrive] = rightSpeed * 0.69;
		motor[leftLift] = motor[rightLift] = -30;
		waitUntil(getMotorEncoder(rightDrive) > 350);
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
		motor[leftDrive] = motor[rightDrive] = motor[leftLift] = motor[rightLift] = 0;
		flipperEnable = 0;
		flipperUp = false;
		while(SensorValue[flip] < 1200)
			motor[flipper] = 127;
		motor[flipper] = 0;
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
		...
		motor[leftDrive] = -leftSpeed * 0.5;
		motor[rightDrive] = -rightSpeed * 0.69;
		waitUntil(getMotorEncoder(rightDrive) < -480);
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
		if(SensorValue[autonDirection] < 1850)
		{
			motor[leftDrive] = -leftSpeed;
			motor[rightDrive] = rightSpeed;
			waitUntil(getMotorEncoder(rightDrive) > 217);
		}
		else
		{
			motor[leftDrive] = leftSpeed;
			motor[rightDrive] = -rightSpeed;
			waitUntil(getMotorEncoder(leftDrive) > 200);
		}
		motor[leftDrive] = -leftSpeed;
		motor[rightDrive] = -rightSpeed;
		waitUntil(getMotorEncoder(rightDrive) < -900);
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
		int addVal;
		if(SensorValue[autonDirection] < 1850)
		...
		motor[leftDrive] = leftSpeed;
		motor[rightDrive] = rightSpeed;
		motor[leftLift] = motor[rightLift] = 17;
		waitUntil(getMotorEncoder(rightDrive) > 1530 + addVal);
		motor[rightDrive] = motor[leftDrive] = motor[leftLift] = motor[rightLift] = 0;
		resetMotorEncoder(rightDrive);
		resetMotorEncoder(leftDrive);
